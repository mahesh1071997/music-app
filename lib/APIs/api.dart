...
class SaavnAPI {
  List preferredLanguages = Hive.box('settings').get('preferredLanguage', defaultValue: ['Hindi']) as List;
  ...
  Future<Map> getSongFromToken(String token, String type, {int n = 10, int p = 1}) async {
    if (n == -1) {
  ...
  Future<String?> createRadio({required List<String> names, required String stationType, String? language}) async {
    String? params;
  ...
  Future<List> getRadioSongs({required String stationId, int count = 20, int next = 1}) async {
    if (count > 0) {
  ...
  Future<Map> fetchSongSearchResults({required String searchQuery, int count = 20, int page = 1}) async {
  ...
  Future<List<Map>> fetchAlbums({required String searchQuery, required String type, int count = 20, int page = 1}) async {
    String? params;
    if (type == 'playlist') {
  ...
  Future<List> fetchTopSearchResult(String searchQuery) async {
    ...
    if (res.statusCode == 200) {
      final getMain = json.decode(res.body);
      final List responseList = getMain['results'] as List;
      return [await FormatResponse.formatSingleSongResponse(responseList[0] as Map)];
    }
    return List.empty();
  }
