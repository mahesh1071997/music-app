...
class SpotifyApi {
  final List<String> _scopes = ['user-read-private', 'user-read-email', 'playlist-read-private', 'playlist-read-collaborative'];
  ...
  String requestAuthorization() => 'https://accounts.spotify.com/authorize?client_id=$clientID&response_type=code&redirect_uri=$redirectUrl&scope=${_scopes.join('%20')}';
  ...
  Future<List<String>> getAccessToken(String code) async {
    ...
    final Map<String, String> body = {'grant_type': 'authorization_code', 'code': code, 'redirect_uri': redirectUrl};
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return <String>[result['access_token'].toString(), result['refresh_token'].toString()];
      }
    } catch (e) {
      // print('Error: $e');
    }
  ...
  Future<List> getUserPlaylists(String accessToken) async {
    try {
      ...
      final response = await get(path, headers: {'Authorization': 'Bearer $accessToken', 'Accept': 'application/json'});
      if (response.statusCode == 200) {
  ...
  Future<Map> getTracksOfPlaylist(String accessToken, String playListId, int offset) async {
    try {
