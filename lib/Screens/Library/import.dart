...
class ImportPlaylist extends StatelessWidget {
  ...
  final List playlistNames = Hive.box('settings').get('playlistNames')?.toList() as List? ?? ['Favorite Songs'];
...
void importSpotify(BuildContext context, List playlistNames, Box settingsBox) {
  ...
  AppLinks(onAppLink: (Uri uri, String link) async {
    closeInAppWebView();
    if (link.contains('code=')) {
      code = link.split('code=')[1];
      await fetchPlaylists(code, context, playlistNames, settingsBox);
    }
  });
}
  ...
  await showTextInputDialog(
    ...
    onSubmitted: (value) async {
      ...
      if (data.isNotEmpty) {
        ...
        while (playlistNames.contains(playName) || await Hive.boxExists(value)) {
...
Future<void> importJioSaavn(BuildContext context, List playlistNames, Box settingsBox) async {
  await showTextInputDialog(
    context: context,
    title: AppLocalizations.of(context)!.enterPlaylistLink,
    initialText: '',
...
Future<void> fetchPlaylists(String code, BuildContext context, List playlistNames, Box settingsBox) async {
        ...
        return BottomGradientContainer(
          child: ListView.builder(
            ...
            itemBuilder: (ctxt, idx) {
                  ...
                  onTap: () async {
                    await showTextInputDialog(
                      ...
                      onSubmitted: (String value) async {
                        ...
                        final Map data = await SpotifyApi().getTracksOfPlaylist(accessToken, value, 0);
                        ...
                        Stream<Map> songsAdder() async* {
                          ...
                          for (final track in tracks) {
                            ...
                            try {
                              final List result = await SaavnAPI().fetchTopSearchResult(
              ...
              final String playName = spotifyPlaylists[idx - 1]['name'].toString().replaceAll('/', ' ');
              ...
              return playTotal == 0
                  ...
                  : ListTile(
                      title: Text(playName),
                      subtitle: Text(
                        playTotal == 1 ? '$playTotal ${AppLocalizations.of(context)!.song}' : '$playTotal ${AppLocalizations.of(context)!.songs}',
                      ),
                      leading: Card(
                        elevation: 8,
                        ...
                        child: (spotifyPlaylists[idx - 1]['images'] as List).isEmpty
                            ...
                            : CachedNetworkImage(
                                ...
                                imageUrl: '${spotifyPlaylists[idx - 1]["images"][0]['url'].replaceAll('http:', 'https:')}',
    ...
    if (index != null) {
      ...
      Stream<Map> songsAdder() async* {
        ...
        for (final track in tracks) {
          ...
          try {
            final List result = await SaavnAPI().fetchTopSearchResult('$trackName by $trackArtist');
